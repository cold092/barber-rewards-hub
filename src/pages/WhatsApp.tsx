...
export default function WhatsApp() {
  ...
  return (
    <DashboardLayout>
      <motion.div
        className="h-[calc(100vh-5rem)] lg:h-[calc(100vh-2rem)] flex flex-col"
        initial="hidden"
        animate="show"
        variants={{ hidden: {}, show: { transition: { staggerChildren: 0.08 } } }}
      >
        {/* Title bar */}
        <motion.div variants={{ hidden: { opacity: 0, y: 16 }, show: { opacity: 1, y: 0 } }} className="flex items-center gap-3 mb-4">
        ...
        {/* Global Tag Filter */}
        <motion.div variants={{ hidden: { opacity: 0, y: 12 }, show: { opacity: 1, y: 0 } }} className="mb-3">
          <GlobalTagFilter tagOptions={crmTags} />
        </motion.div>

        {/* Main chat layout */}
        <motion.div variants={{ hidden: { opacity: 0, y: 12 }, show: { opacity: 1, y: 0 } }} className="flex-1 glass-card rounded-2xl overflow-hidden flex min-h-0">
          {/* Conversation List */}
          <div className={cn(
            "w-80 shrink-0",
